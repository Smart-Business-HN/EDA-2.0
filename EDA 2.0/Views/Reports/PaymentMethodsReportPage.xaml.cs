...
{
    public sealed partial class PaymentMethodsReportPage : Page
    {
        private readonly IMediator _mediator;
        private readonly IReportPdfService _pdfService;
        private PaymentMethodsReportData? _currentData;

        public PaymentMethodsReportPage()
        {
            InitializeComponent();
        ...
        private async Task LoadDataAsync()
        {
            ...
            try
            {
                ...
                var result = await _mediator.Send(new GetPaymentMethodsReportQuery
                {
                    FromDate = fromDate,
                    ToDate = toDate
                });
        ...
        private void UpdateUI(PaymentMethodsReportData data)
        {
            TotalTransactionsText.Text = data.TotalTransactions.ToString();
            TotalAmountText.Text = $"L {data.GrandTotal:N2}";

            var displayItems = data.Items.Select(i => new PaymentMethodDisplayItem
            {
        ...
        private async void PrintPdf_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                var pdfBytes = _pdfService.GeneratePaymentMethodsReportPdf(_currentData);
                var tempPath = Path.Combine(Path.GetTempPath(), $"ReporteMetodosPago_{DateTime.Now:yyyyMMdd_HHmmss}.pdf");
    ...
    public class PaymentMethodDisplayItem
    {
        ...
        public string TotalAmountFormatted => $"L {TotalAmount:N2}";
        public string PercentageFormatted => $"{Percentage:N1}%";
    }
}
